...
export const createOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    for (const item of items) {
      ...
      if (!itemIsPreOrder && !hasSufficientStock) {
        res.status(400).json({
          success: false,
          message:
            `This item is not available for pre-order, so your order cannot be placed right now. ` +
    ...
    try {
      const { subject, html } = emailTemplates.orderSubmitted(req.user!.name, order);
...
export const updateOrderStatus = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
      ...
      for (const item of preorderItems) {
        ...
        if (!product || product.stock < item.quantity) {
          ...
          res.status(400).json({
            success: false,
            message:
              `Cannot mark this order as "${status}". Stock is not available for pre-order item "${item.name}". ` +
    ...
    try {
      ...
      if (status === 'pending') {
        const { subject, html } = emailTemplates.orderSubmitted(user.name, order);
        await sendEmail({ to: user.email, subject, html });
      } else if (status === 'confirmed') {
        const { subject, html } = emailTemplates.orderConfirmed(user.name, order);
        await sendEmail({ to: user.email, subject, html });
      } else if (status === 'processing') {
        const { subject, html } = emailTemplates.orderApproved(user.name, order);
        await sendEmail({ to: user.email, subject, html });
      } else if (status === 'shipped') {
        const { subject, html } = emailTemplates.orderShipped(user.name, order, trackingNumber);
        await sendEmail({ to: user.email, subject, html });
      } else if (status === 'delivered') {
        const { subject, html } = emailTemplates.orderDelivered(user.name, order);
        await sendEmail({ to: user.email, subject, html });
      } else if (status === 'cancelled') {
        const { subject, html } = emailTemplates.orderCancelled(user.name, order, note);
        await sendEmail({ to: user.email, subject, html });
      }
    } catch (e) { console.error('Email error:', e); }
    res.json({ success: true, order });
...
export const cancelOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await order.save();
    try {
      const user = req.user as unknown as { name: string; email: string };
      const { subject, html } = emailTemplates.orderCancelled(user.name, order, req.body.reason);
      await sendEmail({ to: user.email, subject, html });
    } catch (e) { console.error('Email error:', e); }
    res.json({ success: true, order });
  } catch (error) { res.status(500).json({ success: false, message: (error as Error).message }); }
...
export const deleteOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!isOwner && !isAdmin) { res.status(403).json({ success: false, message: 'Not authorized' }); return; }
